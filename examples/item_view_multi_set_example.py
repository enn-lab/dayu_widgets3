...
class ItemViewFullSetExample(QtWidgets.QWidget, MFieldMixin):
    ...
    def _init_ui(self):
        ...
        item_view_set_table = MItemViewMultiSet(table_view=True)
        item_view_set_table.set_header_list(self.header_list)
        item_view_set_table.searchable().groupable()

        item_view_set_all = MItemViewMultiSet(
            table_view=True,
            big_view=True,
            tree_view=True,
            list_view=True
        )
        item_view_set_all.set_header_list(self.header_list)
        item_view_set_all.searchable().groupable()
    ...
    def slot_update_data(self):
        for view in self.view_list:
            view.setup_data(self.data_list)
